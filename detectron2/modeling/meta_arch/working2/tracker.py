                ...
                if(self.use_appearance==True):
                    if(self.use_embed==True):
                        ...
                        if(self.use_color==True):
                            color_dist = np.linalg.norm(np.array(dets[ipred].color)-np.array(self.tracks[itrack].color),ord=1)/175
                iou_overlap = iou(dets[ipred].corners(),tracks[itrack].corners())
                iou_dist = 1-iou_overlap
                self.distances.append([self.frameCount,color_dist,iou_dist,dets[ipred].corners(),tracks[itrack].corners()])
                #uncertainety =np.maximum(1-dets[ipred].conf,0.5)
                total_dist = iou_dist +desc_dist+color_dist
                #if(desc_dist>8):
    ...
    def track(self,dets_org,descs_tensor ,frame,prev_frame_gray):
        ...
        for i in np.arange(len(dets_tensor.pred_boxes)):
            ...
            if(self.use_appearance==True and  self.use_embed==False):
                ...
                if(self.use_color==True):
                    dets[i].calc_color_descriptor(frame)
            else:
                dets[i].hog=None
                ...
                if(t not in c ):
                    ...
                    if(self.use_overlap==True):
                        for others in self.tracks:
                            if(trk.track_id==others.track_id or trk.missed_count>1):
                                continue
                            if(ios(trk.corners(),others.corners())>=0.5):
                                trk.is_overlap = True
                                break
                    trk.update(dets_class[np.where(c==t)[0][0]],None,None)
                    matched +=1
            for d,det in enumerate(dets_class):
                if(d not in r and det.conf>0.5):
                    missed_dets+=1
