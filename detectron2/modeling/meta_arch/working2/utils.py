...
def ios(a,b):
	...
	iou = area_overlap / (area_combined+epsilon)
	return iou   
def get_color_descriptor(frame,xmin,ymin,xmax,ymax):
    res = []
    if(xmin<0):
        xmin = 0
    if(ymin<0):
    ...
    if(section.shape[0]==0 or section.shape[1]==0):
        return np.zeros(9)
    hsv = cv2.cvtColor(section, cv2.COLOR_BGR2Lab)
    hist,_ = np.histogram(hsv[:,:,0],bins=64)
    ind = np.argpartition(hist, -3)[-3:]
    #med = [np.median(hsv[:,:,0]),np.median(hsv[:,:,1]),np.median(hsv[:,:,2])]
    res.extend(ind)
    hist,_ = np.histogram(hsv[:,:,1],bins=64)
    ind = np.argpartition(hist, -3)[-3:]
    
    res.extend(ind)
    hist,_ = np.histogram(hsv[:,:,2],bins=64)
    ind = np.argpartition(hist, -3)[-3:]
